...
import { Wallet, Bell, ShieldCheck, Activity, Eye, EyeOff, LockKeyhole, ArrowRight, LayoutDashboard, Crown, Coins, Landmark, ShieldPlus } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-300 max-w-7xl mx-auto w-full">
      <header className="flex flex-col gap-1 focus-visible:outline-none" tabIndex={-1}>
        <h1 className="text-3xl font-black text-white tracking-tight">Workspace</h1>
        <p className="text-slate-300 text-sm">Welcome back, <span className="text-white font-semibold">@{user?.username}</span>.</p>
      </header>

      <Tabs defaultValue="overview" className="w-full mt-2">
        {/* UI/UX: Horizontal Scrollable Tabs for Mobile, Grid for Desktop */}
        <TabsList className="flex w-full overflow-x-auto custom-scrollbar justify-start sm:grid sm:grid-cols-5 bg-[#11141d]/80 backdrop-blur-md border border-slate-800/80 mb-6 p-1.5 rounded-2xl h-auto shrink-0">
          <TabsTrigger value="overview" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white rounded-xl py-2.5 px-4 font-semibold transition-all shrink-0">
            <LayoutDashboard size={16} className="mr-2 hidden sm:block md:hidden lg:block" /> Overview
          </TabsTrigger>
          <TabsTrigger value="vip" className="data-[state=active]:bg-amber-600 data-[state=active]:text-white rounded-xl py-2.5 px-4 font-semibold transition-all shrink-0">
            <Crown size={16} className="mr-2 hidden sm:block md:hidden lg:block" /> VIP Center
          </TabsTrigger>
          <TabsTrigger value="earn" className="data-[state=active]:bg-emerald-600 data-[state=active]:text-white rounded-xl py-2.5 px-4 font-semibold transition-all shrink-0">
            <Coins size={16} className="mr-2 hidden sm:block md:hidden lg:block" /> Earn
          </TabsTrigger>
          <TabsTrigger value="withdraw" className="data-[state=active]:bg-purple-600 data-[state=active]:text-white rounded-xl py-2.5 px-4 font-semibold transition-all shrink-0">
            <Landmark size={16} className="mr-2 hidden sm:block md:hidden lg:block" /> Withdraw
          </TabsTrigger>
          <TabsTrigger value="support" className="data-[state=active]:bg-rose-600 data-[state=active]:text-white rounded-xl py-2.5 px-4 font-semibold transition-all shrink-0">
            <ShieldPlus size={16} className="mr-2 hidden sm:block md:hidden lg:block" /> Support
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="overview" className="focus-visible:outline-none m-0">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-900/50 via-[#131b2f] to-[#0d1017] border border-blue-500/30 p-8 shadow-[0_15px_40px_-15px_rgba(37,99,235,0.25)]">
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    {user?.isVip && (
                      <Badge aria-label="VIP Status Active" className="bg-amber-500/20 text-amber-300 border border-amber-500/40">VIP ACTIVE</Badge>
                    )}
                  </div>
                  <div className="flex items-baseline gap-2 mb-8">
                    <span className="text-6xl font-black text-white tracking-tighter drop-shadow-lg">{(user?.credits ?? 0).toLocaleString()}</span>
              ...
              <section className="bg-[#11141d] border border-slate-800/80 rounded-3xl p-6 shadow-xl flex-1 flex flex-col">
                <header className="flex items-center justify-between mb-6 pb-4 border-b border-slate-800/50">
                  <div className="flex items-center gap-3">
                    <div className="p-2 bg-emerald-500/10 rounded-lg"><LockKeyhole size={20} className="text-emerald-400" /></div>
                    <div>
                      <h3 className="text-lg font-bold text-white">Your Collection</h3>
                      <p className="text-sm text-slate-400">Your purchased assets</p>
                    </div>
                  </div>
                  <Badge variant="outline" className="bg-black/50 text-slate-300 border-slate-700">{purchases.length} Assets</Badge>
                </header>
                    ...
                    purchases.map((p, i) => (
                      <article key={i} className="bg-black/60 border border-slate-800 rounded-2xl p-4 hover:border-slate-600 transition-colors group focus-within:ring-2 focus-within:ring-emerald-500/50 outline-none">
                        ...
                        <div className="relative mt-2">
                          <div className={`p-3 rounded-xl border font-mono text-sm break-all transition-all duration-200 ${revealedAssets[i] ? 'bg-emerald-950/30 border-emerald-500/40 text-emerald-300 select-all' : 'bg-slate-900 border-slate-800 text-transparent select-none blur-sm'}`}>
                            {p.assetData}
                          </div>
                          <Button 
                            variant="ghost" size="sm" onClick={() => toggleReveal(i)}
            ...
            {/* RIGHT COLUMN: Network Activity Feed */}
              ...
              <section className="bg-[#11141d] border border-slate-800/80 rounded-3xl p-6 shadow-xl h-full flex flex-col">
                ...
                <div className="flex flex-col gap-4 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-800 before:to-transparent">
                  {dataLoading ? <ActivitySkeletons /> : latestNews.length === 0 ? (
                    ...
                    latestNews.map((news) => (
                      <article key={news.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                        <div className="flex items-center justify-center w-10 h-10 rounded-full border-4 border-[#11141d] bg-slate-900 text-slate-500 group-hover:bg-purple-600 group-hover:text-white shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 transition-colors z-10"><Bell size={14} /></div>
                        <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-2xl bg-[#0a0c10] border border-slate-800/80 hover:border-purple-500/40 transition-colors shadow-sm focus-within:ring-2 focus-within:ring-purple-500 outline-none">
                          <div className="flex flex-col gap-1">
                            ...
                            <time className="text-[10px] text-slate-500 font-mono mt-2">{new Date(news.createdAt).toLocaleDateString()}</time>
                          </div>
                        </div>
                      </article>
                    ))
        ...
        {/* Dynamic Panels - Rendered Only When Tab is Active */}
        <Suspense fallback={<DashboardSkeleton />}>
          <TabsContent value="vip" className="focus-visible:outline-none"><VIPPanel /></TabsContent>
          <TabsContent value="earn" className="focus-visible:outline-none"><EarnPanel /></TabsContent>
          <TabsContent value="withdraw" className="focus-visible:outline-none"><WithdrawPanel /></TabsContent>
          <TabsContent value="support" className="focus-visible:outline-none"><SupportPanel /></TabsContent>
        </Suspense>
      </Tabs>
    </div>
  );
...
const DashboardSkeleton = () => (
  <div className="flex flex-col gap-8 max-w-7xl mx-auto w-full animate-pulse">
    <div><Skeleton className="h-10 w-48 mb-2 bg-slate-800" /><Skeleton className="h-4 w-64 bg-slate-800" /></div>
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
...
const VaultSkeletons = () => <div className="flex flex-col gap-4">{[1, 2].map(i => <Skeleton key={i} className="h-32 w-full rounded-2xl bg-slate-800/50" />)}</div>;
const ActivitySkeletons = () => <div className="flex flex-col gap-6 w-full">{[1, 2, 3].map(i => <Skeleton key={i} className="h-24 w-full rounded-2xl bg-slate-800/50" />)}</div>;
